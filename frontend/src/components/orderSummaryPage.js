...
export default function OrderSummaryPage() {
  ...
  // Load showtime + ticket price
  useEffect(() => {
    if (!showtimeId) return;

    axios.get(`http://localhost:9090/api/showtimes/showtime/${showtimeId}`)
      .then((res) => setBasePrice(res.data.ticketPrice))
      .catch((err) => console.error("Failed to load showtime:", err));
  }, [showtimeId]);

  // Load online fee
  useEffect(() => {
    axios.get("http://localhost:9090/settings/fee")
      .then((res) => setOnlineFee(res.data))
      .catch(() => setOnlineFee(0));
  }, []);
  ...
  // Initialize ticketTypes = ADULT for each seat
  useEffect(() => {
  ...
  // Wait for ticket price
  if (basePrice == null) return <div className="summary-container">Loading prices...</div>;
          ...
          {selectedSeats.map((id) => (
            <li key={id} className="seat-item">
              ...
              >
                ...
                <option value="CHILD">CHILD — ${(basePrice * 0.65).toFixed(2)}</option>
                <option value="SENIOR">SENIOR — ${(basePrice * 0.80).toFixed(2)}</option>
              </select>
            </li>
          ))}
        </ul>
      ...
      <div className="billing-card">
        ...
        {selectedBilling ? (
          <>
            ...
            {selectedBilling.default && <p><em>(Default payment method)</em></p>}
          </>
        ) : (
