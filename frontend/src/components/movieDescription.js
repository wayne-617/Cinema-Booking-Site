...
export default function MovieDescription() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) setMState({ loading: false, error: e.message || "Failed to load movie" });
      }
    }
    load();
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) setSState({ loading: false, error: e.message || "Failed to load showtimes" });
      }
    }
    load();
    return () => { cancelled = true; };
  ...
  // normalize fields
  const poster = movie?.poster_url || "https://via.placeholder.com/600x900?text=No+Poster";
  const title  = movie?.title || "Movie";
  const rating = movie?.rating ?? movie?.mpaaRating ?? movie?.mpaa_rating ?? "NR";
  ...
  const grouped = useMemo(() => {
    ...
    Object.values(g).forEach(arr => arr.sort((a,b)=> (a.showTime || "").localeCompare(b.showTime || "")));
    return g;
  }, [myShowtimes]);
  ...
  if (mState.loading) {
    return (
      <div className="moviesPageContainer">
        <div className="movie-card skeleton" style={{height:180, borderRadius:12}} />
      </div>
    );
  }
  if (mState.error) {
    return (
      <div className="moviesPageContainer">
        <div className="alert alert-danger">Failed to load movie: {mState.error}</div>
        <button className="btn btn-outline-light" onClick={() => nav(-1)}>Back</button>
      </div>
    );
  }
      ...
      <div className="movieDetailSection">
        <div className="trailerWrap">
          {trailerYt ? (
            ...
            <div className="trailerFallback">
              <a href={trailer} target="_blank" rel="noreferrer">Open trailer</a>
            </div>
          ) : (
            <div className="trailerFallback">
        ...
        <div className="detailContent">
          ...
          <div className="detailMeta">
            <span>{rating}</span><span className="dot">•</span><span>{genre}</span>
          </div>

          <div className="detailsGrid">
            <div className="detailBlock">
              <h3>Overview</h3>
              <div className="detailText">{movie?.synopsis || "No synopsis provided."}</div>
            </div>
            <div className="detailBlock">
              <h3>Credits</h3>
              <div><span className="detailLabel">Director:</span> <span className="detailText">{movie?.director || "—"}</span></div>
              <div><span className="detailLabel">Producer:</span> <span className="detailText">{movie?.producer || "—"}</span></div>
              <div><span className="detailLabel">Cast:</span> <span className="detailText">{cast}</span></div>
            </div>
          </div>
              ...
              <div className="showtimesGrid">
                {dateKeys.map(d => (
                  <div className="showDateCol" key={d}>
                    <h4>{fmtDate(d)}</h4>
                    {grouped[d].map(s => (
                      <button key={s.showtimeId} className="timeBtn" type="button">
